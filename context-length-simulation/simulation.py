...
def stream_chat(prompts, i):
    ...
    for prompt in prompts:
                ...
                for line in r.iter_lines():
                    if line and line.startswith(b"data:"):
                        try:
                            ...
                            # print(f"User {i} Prompt {promot_count}: ", chunk['choices'][0]['delta'].get('content', ''), "\n", end='', flush=True)
        ...
        total_context_length += response_tokens
        ...
        with open(f"/workspace/user_{i}_prompt_{promot_count}.txt", "w") as f:
            f.write(f"{response}\n\n\n duration: {duration}, T/s: {str(response_tokens/duration)}, conversation context length: {conversation_context_length}")
